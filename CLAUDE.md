...
- **`--seed-from <dir>` (local time-saver, not the reproducible path):** hash-matches files
  already on ESS (e.g. `/fs/ess/PAS2254/Cartographer_Data/Unmodified/ProteomeTools/{ds}/raw/`)
  and hardlinks/moves them in instead of downloading. Equivalent to a manual `mv`.
- **Canonical data root (ESS):** `/fs/ess/PAS2254/DBW/protostar/data/raw/{dataset}/`. mzpeak
...
- **Project dir (ESS):** `/fs/ess/PAS2254/DBW/protostar/` (data root, results, job logs). Do
  **not** store project files in OSC home (strict storage/file-count limits).
- **Account/cluster:** account `PAS2254`, home account `PAS1309`, default cluster Cardinal
